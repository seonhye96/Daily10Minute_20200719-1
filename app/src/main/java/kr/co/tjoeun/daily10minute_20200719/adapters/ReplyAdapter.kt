...
//        좋아요 갯수 처리 -> 0개면 숨김, 그 이상이면 보여주고 갯구반영
        if(data.likeCount == 0){
...
//            빨간 글씨 => colors.xml 에 있는 항목 (naverRed) 가져오기
            likeBtn.setTextColor(mContext.getColor(R.color.naverRed))

//            좋아요 취소로 문구 변경
            likeBtn.text = "좋아요 취소"
        }else{
            likeBtn.setBackgroundColor(R.drawable.gray_border_box)
//            검정글씨 => 기본색에서 BLACK 지원 (기본 색 사용법 예시)
...
//        텍스트뷰 / 이밎뷰 / 리니어레이아웃 등의 특이한 이벤트가 없는 모든 뷰들은
